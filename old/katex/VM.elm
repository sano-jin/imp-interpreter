...
showAExpState (aExp, state) =
    "\\langle " ++ IP.showAExp aExp 10 ++ ", \\{ " ++ showState state ++ " \\} \\rangle"

showAExpTrans : AExpTrans -> TransString
showAExpTrans (aExpState, num, aExpRule) =
    Trans
    (showAExpState aExpState ++ " \\longrightarrow " ++ String.fromInt num)
...
showBExpState (bExp, state) =
    "\\langle " ++ IP.showBExp bExp ++ ", \\{ " ++ showState state ++ " \\} \\rangle"

showBExpTrans : BExpTrans -> TransString
showBExpTrans (bExpState, bool, bExpRule) =
    let boolStr = if bool then " \\texttt{ture} " else " \\texttt{false} " in
    Trans
    (showBExpState bExpState ++ " \\longrightarrow " ++ boolStr)
